(function () {
  ...
  function updateStats() {
    ...
    if (investedSub) {
      ...
      investedSub.textContent = parts.length > 0 ? parts.join(' · ') : 'stocks · crypto';
    }
  }
  ...
  function setupGuide() {
    ...
    function close() { overlay.style.display = 'none'; try { localStorage.setItem('mm_seen_guide', '1'); } catch (e) { /* ignore */ } }
    ...
    // First visit: auto-open once the login screen (if any) is gone.
    var seen = null;
    try { seen = localStorage.getItem('mm_seen_guide'); } catch (e) { /* ignore */ }
    if (!seen) {
      var tries = 0;
      var t = setInterval(function () {
        tries++;
        var gate = document.getElementById('auth-gate');
        var gateVisible = gate && gate.style.display !== 'none';
        if (!gateVisible) { clearInterval(t); open(); }
  ...
  // --- Export / Import data ---
  var DATA_KEYS = ['mm_crypto', 'mm_stocks', 'mm_cash', 'mm_portfolio_snapshots', 'mm_name', 'mm_current_page'];
  ...
  function init() {
    ...
    // looks alive instantly (great when someone opens it from a portfolio/CV).
    try {
      if (!localStorage.getItem('gallop_started') && localStorage.getItem('gallop_was_authed') !== '1') {
        localStorage.setItem('gallop_started', '1');
        setTimeout(function () { if (typeof window.__gallopLoadSample === 'function') window.__gallopLoadSample(); }, 700);
      }
    } catch (e) { /* ignore */ }
