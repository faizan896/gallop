(function () {
  ...
  async function fetchSPX() {
    try {
      // Finnhub's free tier doesn't serve the S&P 500 index (^GSPC), so we use
      // the SPY ETF as a proxy. SPY tracks ~1/10th of the index, so multiply by
      // 10 to approximate the actual S&P 500 index level. (Change % is identical.)
      var q = await fetchFinnhubQuote('SPY');
      var spxLevel = q.price * 10;
      var el = document.getElementById('widget-price-spx');
      if (el) { el.textContent = fmtPrice(spxLevel, 2); el.classList.remove('skeleton-text'); }
